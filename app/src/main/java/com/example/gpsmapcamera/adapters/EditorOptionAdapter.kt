...
//            VIEW_TYPE_FONT -> PrefManager.getInt(context, PrefManager.KEY_TYPE_FONT,RecyclerView.NO_POSITION)
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): EditorOptionViewHolder {
        val binding = when (viewType) {
            VIEW_TYPE_COLOR -> EditorOptionAdapterLayoutBinding.inflate(LayoutInflater.from(parent.context), parent, false)
            VIEW_TYPE_BGCOLOR -> EditorOptionAdapterLayoutBinding.inflate(LayoutInflater.from(parent.context), parent, false)
            VIEW_TYPE_FONT -> EditorOptionAdapterLayoutBinding.inflate(LayoutInflater.from(parent.context), parent, false)
    ...
    override fun onBindViewHolder(holder: EditorOptionViewHolder, position: Int) {
        holder.bind(items[position], position, selectedIndex) { newIndex,type ->
            ...
            when(type)
            {
                VIEW_TYPE_COLOR -> PrefManager.saveInt(holder.itemView.context, PrefManager.KEY_TYPE_COLOR,selectedIndex)
                VIEW_TYPE_BGCOLOR -> PrefManager.saveInt(holder.itemView.context, PrefManager.KEY_TYPE_BG_COLOR,selectedIndex)
                VIEW_TYPE_FONT -> PrefManager.saveInt(holder.itemView.context, PrefManager.KEY_TYPE_FONT,selectedIndex)
            }
        ...
        fun bind(
            ...
            when (item) {
                ...
                is EditorOption.BGColorOption -> {
                    ...
                    {
                        0->{
                            colorBinding.colorCircle.setImageResource(R.drawable.ic_close)
                        }
                        else -> {
                            colorBinding.colorCircle.setImageResource(0)
                        }
                    }
                    colorBinding.root.setOnClickListener {
                ...
                is EditorOption.FontOption -> {
                    ...
                    fontBinding.fontPreview.setTextColorRes( if (position == selectedIndex) {
                        R.color.blue
                    } else  R.color.white)
