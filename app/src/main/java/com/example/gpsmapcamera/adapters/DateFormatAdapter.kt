...
class DateFormatAdapter(
    private val fromTimeZone: Boolean,
    private val formats: List<String>,
    private val passedTemplate : String,
    private val onItemClick: (Int) -> Unit
) : RecyclerView.Adapter<DateFormatAdapter.ViewHolder>() {
    ...
    inner class ViewHolder(private val binding: SingleDateFormatBinding) : RecyclerView.ViewHolder(binding.root) {
        init {
            selectedPosition = getInt(binding.root.context, if(fromTimeZone) Constants.TIME_ZONE_SELECTED_FORMAT + passedTemplate else Constants.DATE_TIME_SELECTED_FORMAT + passedTemplate, 0)
        }
        fun bind(format: String, position: Int) = binding.run {
            tvMain.text = if (fromTimeZone) format else Date().formatToString(format)
